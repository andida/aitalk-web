...
export function getMetadata(
  ...
  return async function generateMetadata({
    ...
    const languages = await getLanguageAlternates(options.canonicalUrl || '');
...
async function getLanguageAlternates(canonicalUrl: string) {
  const alternates: Record<string, string> = {};

  for (const locale of locales) {
    alternates[locale] = await getCanonicalUrl(canonicalUrl, locale);
  }

  alternates['x-default'] = await getCanonicalUrl(canonicalUrl, defaultLocale);
