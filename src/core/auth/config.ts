...
} from '@/shared/blocks/email/verify-email';
import {
  getCookieFromCtx,
  getHeaderValue,
...
export async function getAuthOptions(configs: Record<string, string>) {
  ...
  if (!envConfigs.database_url) {
    missingAuthEnv.push('DATABASE_URL');
  }
  ...
  if (missingAuthEnv.length > 0) {
    throw new Error(
      `Auth runtime is not configured. Missing ${missingAuthEnv.join(
        ', '
      )}. Set them in the deployment environment before using auth endpoints.`
    );
  }
          ...
          emailVerification: {
            ...
            sendVerificationEmail: async (
              ...
              try {
                ...
                if (!result.success) {
                  throw new Error(
                    `Email provider "${result.provider}" failed: ${
