...
export const siteriseDomain = table(
  'siterise_domain',
  {
    id: text('id').primaryKey(),
    ...
    source: text('source').notNull().default('query_domains'),
  ...
  (table) => [
    index('idx_siterise_domain_tld').on(table.tld),
    index('idx_siterise_domain_category').on(table.category),
    index('idx_siterise_domain_first_seen').on(table.firstSeenAt),
    index('idx_siterise_domain_registered').on(table.registeredAt),
    index('idx_siterise_domain_status').on(table.status),
  ]
);

export const siteriseTrafficSnapshot = table(
  'siterise_traffic_snapshot',
  {
    id: text('id').primaryKey(),
    domainId: text('domain_id')
      .notNull()
      .references(() => siteriseDomain.id, { onDelete: 'cascade' }),
    rootDomain: text('root_domain').notNull(),
    displayDate: date('display_date').notNull(),
    country: text('country').notNull().default('global'),
    deviceType: text('device_type').notNull().default('all'),
    source: text('source').notNull().default('query_domains'),
    visits: bigint('visits', { mode: 'number' }),
    users: bigint('users', { mode: 'number' }),
    desktopVisits: bigint('desktop_visits', { mode: 'number' }),
    mobileVisits: bigint('mobile_visits', { mode: 'number' }),
    bounceRate: doublePrecision('bounce_rate'),
    pagesPerVisit: doublePrecision('pages_per_visit'),
    timeOnSite: integer('time_on_site'),
    accuracy: text('accuracy'),
    raw: jsonb('raw').$type<Record<string, unknown>>(),
    fetchedAt: timestamp('fetched_at').defaultNow().notNull(),
    cacheUntil: timestamp('cache_until'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at')
      .$onUpdate(() => new Date())
      .notNull(),
  ...
  (table) => [
    uniqueIndex('uidx_siterise_snapshot_domain_month_market').on(
      table.domainId,
      table.displayDate,
      table.country,
      table.deviceType,
    ...
    index('idx_siterise_snapshot_month_country_visits').on(
      table.displayDate,
      table.country,
      table.visits
    ),
    index('idx_siterise_snapshot_root_domain').on(table.rootDomain),
    index('idx_siterise_snapshot_cache_until').on(table.cacheUntil),
  ]
);

export const siteriseDomainWhoisCache = table(
  'siterise_domain_whois_cache',
  {
    id: text('id').primaryKey(),
    rootDomain: text('root_domain').notNull().unique(),
    ...
    registeredAt: timestamp('registered_at'),
    expiresAt: timestamp('expires_at'),
    domainUpdatedAt: timestamp('domain_updated_at'),
    ...
    cacheUntil: timestamp('cache_until').notNull(),
  ...
  (table) => [
    index('idx_siterise_whois_root_domain').on(table.rootDomain),
    index('idx_siterise_whois_cache_until').on(table.cacheUntil),
    index('idx_siterise_whois_registered_at').on(table.registeredAt),
  ]
);

export const siteriseLeaderboardRun = table(
  'siterise_leaderboard_run',
  {
    ...
    category: text('category'),
    source: text('source').notNull().default('query_domains'),
    minVisits: integer('min_visits').notNull().default(0),
  ...
  (table) => [
    uniqueIndex('uidx_siterise_leaderboard_run_scope').on(
      table.kind,
      table.displayDate,
      table.country,
      table.category,
    ...
    index('idx_siterise_leaderboard_run_status').on(table.status),
  ]
);

export const siteriseLeaderboardEntry = table(
  'siterise_leaderboard_entry',
  {
    id: text('id').primaryKey(),
    runId: text('run_id')
      .notNull()
      .references(() => siteriseLeaderboardRun.id, { onDelete: 'cascade' }),
    domainId: text('domain_id')
      .notNull()
      .references(() => siteriseDomain.id, { onDelete: 'cascade' }),
    rootDomain: text('root_domain').notNull(),
    ...
    visits: bigint('visits', { mode: 'number' }),
    previousVisits: bigint('previous_visits', { mode: 'number' }),
    visitsDelta: bigint('visits_delta', { mode: 'number' }),
  ...
  (table) => [
    uniqueIndex('uidx_siterise_leaderboard_entry_run_domain').on(
      table.runId,
      table.domainId
    ),
    index('idx_siterise_leaderboard_entry_rank').on(table.runId, table.rank),
    index('idx_siterise_leaderboard_entry_growth').on(
      table.runId,
      table.growthRate
    ),
    index('idx_siterise_leaderboard_entry_root_domain').on(table.rootDomain),
  ]
);

export const siteriseWatchlist = table(
  'siterise_watchlist',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
      .notNull()
      .references(() => user.id, { onDelete: 'cascade' }),
    domainId: text('domain_id').references(() => siteriseDomain.id, {
      onDelete: 'set null',
    }),
    rootDomain: text('root_domain').notNull(),
    label: text('label'),
  ...
  (table) => [
    uniqueIndex('uidx_siterise_watchlist_user_domain_country').on(
      table.userId,
      table.rootDomain,
      table.country
    ),
    index('idx_siterise_watchlist_user_status').on(table.userId, table.status),
    index('idx_siterise_watchlist_domain').on(table.rootDomain),
  ]
);

export const siteriseTrafficQueryLog = table(
  'siterise_traffic_query_log',
  {
    ...
    provider: text('provider').notNull().default('query_domains'),
  ...
  (table) => [
    index('idx_siterise_query_log_user_created').on(
      table.userId,
      table.createdAt
    ),
    index('idx_siterise_query_log_endpoint_created').on(
      table.endpoint,
      table.createdAt
    ),
    index('idx_siterise_query_log_ip_created').on(
      table.ipHash,
      table.createdAt
    ),
  ]
